...
impl<B: Backend> EmbeddingModel<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 2, Int>, mask: Tensor<B, 2, Bool>) -> Tensor<B, 2> {
        ...
        let encoder_input = TransformerEncoderInput::new(hidden).mask_pad(mask.clone());
        let encoded = self.encoder.forward(encoder_input);

        let d_model = self.norm.gamma.dims()[0];
        let mask_f: Tensor<B, 3> = mask
            .float()
            .unsqueeze_dim::<3>(2)
        ...
        let norms: Tensor<B, 2> = projected
            .clone()
            .powf_scalar(2.0)
            .sum_dim(1)
            .sqrt()
